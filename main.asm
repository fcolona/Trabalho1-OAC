...
buffer_tipo_vagao:	.space 12
			...
			# s0 guarda o endereço da locomotiva (PERMANENTE)
			# s1 guarda inteiro input do usuário
			# s2 guarda endereços de vagão
			# s9 dado que será colocado na memória
...
opcao1:			#aloca memória para o novo vagão
			...
			# ASCII ARTE DO VAGÃO SERIA BACANA
...
opcao3:
			...
			addi a7, zero, 5
			ecall #lê um inteiro
			add s1, zero, a0 #guarda no registrador s1
...
remover:	#primeiro, vamos checar se o vagao encontrado é a locomotiva
			...
			lw s4, 28(s2) # carrega em s4 o endereço do vagão depois
			sw s4, 28(s3) # liga o vagão antes ao vagão depois
